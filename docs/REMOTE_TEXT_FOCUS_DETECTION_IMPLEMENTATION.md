...
状态（2026-09-08 收尾）：停止发布自动弹出能力；客户端只保留手动键盘和尽力避让。以下“收尾决策”优先于后续历史设计与 Spike 记录；旧文中的“可信”“权威”“低误报”和“闭环”不代表现有 Windows 检测能力已通过兼容性验收。

## 收尾决策（当前产品行为）

- `RemoteImeController` 不再调用显示 IME、抢焦点或 `restartInput`。UIA 和 InputPane 两个来源都不能自动打开客户端键盘；保留现有手动键盘入口。未提供可误开启的实验开关，后续只有完成明确场景矩阵验证才考虑实验入口。
- 不继续扩展协议，不增加光标或新的启发式检测路径。本次不改 Sunshine 服务、不发布主机组件，已有事件只作为可选的几何提示。
- 停靠键盘以 Android IME Insets 的可见状态和 bottom 遮挡高度为准；明确隐藏时恢复偏移。只有无法获得 Insets 时才使用旧版 visible-frame 回退。
- 浮动键盘可见但 bottom=0 时不移动，不宣称任意第三方浮动键盘避让。
- 避让优先使用捕获画面内、位于元素矩形内的有效 caret；否则使用有匹配标志且在画面内的点击锚点。**不再使用整个元素/Document 的底边**。几何合法不等于元素身份已验证，这仍是尽力提示。
- 缺少主机上下文时手动键盘仍可使用，但不承诺自动定位输入框。没有读取或同步主机输入内容，也不替主机应用滚动文档。
- IME 偏移独立于用户平移/缩放，画面和指针覆盖层同步移动；隐藏、可信失活、重置会话和销毁均清理偏移。

### 用户控制权补充（2026-09-08）

- 键盘由隐藏变为可见时，只消费一次近期上下文；当前阈值为客户端收到后 5 秒，采用单调时钟。它不证明主机快照本身新鲜，不修改现有协议。
- 一个打开周期锁定一个目标，不随新的主机上下文切换目标；可继续按键盘遮挡高度调整。缺少近期目标时不移动，关闭后不会复用上一次目标。
- 生产手势处理器在实际平移或缩放时通知避让控制器接管。保留当时的临时偏移，既不清零造成跳动，也不再抵消用户操作。
- 用户接管后，即使收到目标失活也不立即清零临时偏移；键盘关闭、会话重置或销毁时才撤销。用户的基础平移/缩放仍保留。浮动键盘没有可靠遮挡时仍撤销自动偏移。
- 该策略主动牺牲键盘打开期间自动跟随其他输入框的能力，优先保证画面稳定和用户控制权。

补充验证：17 项策略/周期单元测试、3 项 API 34 模拟器测试通过。生产手势处理器上拖后再次派发相同 Insets，画面保持用户拖动位置；关闭后保留基础平移，无新上下文重新打开不再复用旧目标。实际软键盘显示/隐藏测试仍通过。缩放接管已接入同一通知机制，但尚未单独完成双指手势和动画过程验收。

收缩原因：本机 Chrome 网页空白点击复现了 `Document + TextEditPattern + Value.IsReadOnly=true` 被判为 editable。并发 InputPane 探针始终为空，不能将这次误弹归因于 Windows 触摸键盘。传输链路打通不等于检测准确。

### 验证范围

2026-09-08 执行结果：`testNonRootDebugUnitTest --tests com.limelight.nvstream.RemoteTextContextPolicyTest` 12/12 通过；`RemoteImeAvoidanceTest` 模拟器 instrumentation 2/2 通过；NonRoot Debug 和测试 APK 构建成功。测试版已覆盖安装到模拟器，未安装到实体手机，未修改主机服务。测试构建的 R8 裁剪导致过 AndroidX Builder / Kotlin copy 辅助方法缺失，测试改为系统 Insets 与显式构造后重跑通过，未为测试放宽生产混淆配置。

- 策略单元测试：caret/点击回退、整页矩形、越界坐标、捕获尺寸、停靠遮挡、浮动键盘、坐标原点、重复计算，以及既有协议校验。
- `RemoteImeAvoidanceTest`：模拟器运行生产 Controller、PanZoomHandler 和 Android View，注入确定性的 IME Insets；验证画面/指针同步、重复计算不漂移、浮动与隐藏恢复、会话重置和销毁。
- 另有真实软键盘测试：Android 14/API 34 模拟器（`emulator-5554`），`com.google.android.inputmethod.latin/.LatinIME`。使用生产 Controller/StreamView/PanZoomHandler 和合成主机上下文，显式打开键盘，检查点击锚点在键盘上方、画面与指针同步，隐藏后恢复零偏移。
- 注入 Insets 的测试不是实际浮动键盘窗口测试。其他第三方停靠键盘、动画全过程、分屏/旋转、真实 Sunshine 串流下的菜单手动开关仍需设备验收，不标记为全链路闭环。
