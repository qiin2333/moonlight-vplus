...
2. 用户明确点击“打开键盘”时临时启用输入连接并请求焦点；软键盘从可见变为隐藏后调用 `restartInput()` 并撤销输入连接。API 33+ 上可信的远程文本激活同样走这一临时路径自动开启键盘（默认开启，可关闭）；这些设备的 View 已显式关闭自动手写，临时连接不重新开放画面手写。
...
| 远程文本框获得焦点 | 最新代码记录属性和避让上下文，不自动弹键盘 | API 33+ 自动弹键盘（可关，同一激活仅一次，失焦收起），画面手写保持关闭；API 33 以下维持不弹 |
